...
16. **User-managed ignore: negation scope and anchoring.** (Phase 5b) Because the 13 built-in defaults and the user `.frictionmap-ignore`/`--ignore` patterns are independent matchers OR'd together, a user negation (`!pattern`) in `.frictionmap-ignore` overrides other *user* patterns but cannot un-hide a path matched by a built-in default. User patterns are anchored to the ignore file's directory (or CWD for flag-only runs) and matched against the path relativized to that anchor, so gitignore anchoring (`/build`, `src/foo.py`) behaves as written for in-tree paths; an out-of-tree path (a session that touched a file outside the project root) falls back to absolute matching, where floating patterns (`*.lock`) still fire — gitignore-consistent. Flag (`--ignore`) patterns share the ignore file's anchor when a file is present.
