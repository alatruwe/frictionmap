...
**Structural.** File-intrinsic complexity (cyclomatic complexity, LOC) is computed per file and emitted in the report; the per-LOC value `score_components.normalized_by_loc` is retained as a future secondary lens. As of schema 1.4 it is no longer the headline score — the headline is the raw weighted z-sum (`score_pre_normalization`). Large files do not dominate by surface area regardless, because file size enters via per-rate signals (markers per-100-words, question rate per-100-words) before the weighted sum. See `TRADEOFFS.md` "Scoring normalization: headline is raw weighted z-sum" for the cross-corpus diagnostic that drove the swap.
...
Equal-share (1/N) multi-file weighting was retained after a cross-corpus hand-tag calibration. 30 multi-file-attributed thinking blocks were tagged against a pre-committed disposition rule (15 attune, 15 brownfield; stratified 5 at N=2, 5 at N=3, 5 at N≥4 per corpus): ≥70% "equal share feels right" → keep, 30–70% wrong → flag for v1.x, ≥70% wrong → park. Both corpora cleared keep — attune 11/15, brownfield 12/15 — and equal-share ships unchanged. One observation carries forward: 22/30 blocks (73%) had attribution noise either contaminating the real files or dominating the attributed set, meaning the rule operates on inputs that are noisy more often than not on multi-file blocks; the fixes are upstream of the rule, in the attribution layer's existing v1.x candidates, rather than a different per-file weighting. (The 73% was measured pre-C2-fix, when buggy Tier 1 over-claimed every same-basename file as a high-confidence match — one of the named upstream attribution fixes, landed May 29. Multi-file noise is expected lower now; the figure is not re-measured here. The keep verdict is unaffected — it already located the fixes upstream of the equal-share rule.) The noise profile is corpus-dependent, same shape as the `actually` finding in #4: attune's is mostly Tier 3 over-attribution and bare directory paths; brownfield's is `myapp/X` duplicate path resolution and surviving `.git/` paths.
