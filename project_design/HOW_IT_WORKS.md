...
**Phase 5 #11 — cross-corpus weight calibration (May 2026).** Weight tuning settled with a hand-adjudicated calibration against two reference corpora (attune N=13, brownfield N=12), stratified high/mid/low by per-session friction signal with adversarial-cell kill-test reads. The adjudication rule, pre-committed before any read: a file earns the friction tag if it was edited or read in a stretch where steady progress broke. Files merely mentioned or reasoned about in the broken stretch — but never touched — do not earn the tag, by design, to keep hand-adjudication aligned with the tool's co-location attribution so any disagreement measures weighting rather than attribution semantics.

The 25 entries produced four substantive agreements (3 attune, 1 brownfield) where the rule's tagged file matched the tool's top-ranked file; thirteen calm convergences (6 attune, 7 brownfield) where both produced empty or near-empty output; and six disagreements clustered in three modes: (a) **signal-attribution miscall** — tool elevates files on behavioral signal (edit churn, reread bursts) where reasoning friction is absent, occurring 1× on attune and 3× on brownfield, concentrated in the High bin and consistent with brownfield's denser real-source-edit profile; (b) **thinking-only friction** — the rule produces empty by design when rhythm breaks occur entirely in thinking blocks with no tool_use in their proximity window, occurring 2× on attune and 0× on brownfield; (c) **filter-side exclusion** — `IGNORE_PATTERNS` hid a rule-compliant friction file (`.claude/settings.json`), occurring 1× on attune, expected design behavior of the noise-filter trade. Adversarial-cell kill tests survived on both corpora (attune `7a6d0f49`, brownfield `57dd4ee3` read with intent to disconfirm).

**Disposition: equal weights ship.** None of the six disagreements are weight-tunable: mode (a) is an attribution-shape problem rooted in behavioral signals firing without reasoning-signal gating in some corpora; mode (b) is a definitional limit of the co-location rule; mode (c) is filter-side and already documented under the IGNORE_PATTERNS trade. Equal weights ship because the adjudication produced no evidence weight tuning would change substantive cases. One observation carries forward without being addressed in #11: behavioral-signal over-firing in mode (a) is more pronounced on corpora with dense real-source edits than on doc-heavy corpora, suggesting future work on reasoning-gating refinements rather than weight tuning. New TRADEOFFS entry: "Attribution: co-location rule excludes thinking-only friction" (records mode (b) as accepted gap with reversibility). Calibration ledgers preserved as the durable record.
