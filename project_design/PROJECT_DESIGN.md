...
**Tiered attribution (schema 1.2):** the parser records *how* each block was attributed, not just which files it resolved to. Three tiers in the schema; effectively two in practice:

- `exact_path` (high confidence) — thinking text contains a full path or a `/`-boundary suffix of one matching a session-touched file. Attribution may point to multiple files if the thinking text names multiple distinct paths.
- `unique_basename` (medium) — thinking text names a bare filename with exactly one matching session-touched file. **In practice architecturally subsumed by `exact_path`**: every unique basename is a `/`-boundary suffix of its path, so the `exact_path` branch catches it first. Kept in the schema for clarity; never fires on real data. (Phase 3 may collapse this to a two-tier story in prose.)
