...
four files — three local and gitignored at repo root (`labeling-sheet.md`,
`sample-manifest.csv`, `reserve-list.md`) and one committed
...
def write_seal(pool: list[Unit], excluded_sessions: int,
    ...
    lines = [
        ...
        "#",
        "# Public commitment to a sample whose contents stay local: the three",
        ...
        "# committed and hashed (§4 step 4). The hashes fix the sample now.",
        "#",
    ...
    for path in (SHEET_PATH, SAMPLE_MANIFEST_PATH, RESERVE_PATH):
...
def main() -> int:
    ...
    for path in (SHEET_PATH, SAMPLE_MANIFEST_PATH, RESERVE_PATH, SEAL_PATH):
