...
5. Self-consistency relabel: 20 units from the validation sample, selected by the sampling script's seed, relabeled blind (no access to first-pass labels, signal values, or judge output) no sooner than 48 hours after the first pass completes. Mean absolute disagreement is recorded before any judge run; it sets the Q1 floor (§7) and is reported as a human-vs-human reference alongside κ (§5).
...
Intra-labeler self-consistency (§4 step 5) is reported next to κ as the ceiling on achievable agreement.
...
**Judge model:** **Claude Haiku 4.5, pinned dated snapshot (`claude-haiku-4-5-20251001`), temperature 0.** Extended thinking off (incompatible with temperature 0). Run parameters and the parse-failure rule are bound in `judge-prompts/README.md`, which is a frozen artifact of this registration. Model version change = new pre-registered run. Deployment is single-run per unit; the 3× reruns are validation-only scaffolding.

**Contingency (escalation rule):** escalation fires on any outcome below *validated* — κ < 0.6 or any stability-check miss. Sonnet 5 (`claude-sonnet-5`) becomes the judge and does not inherit Haiku's validation: switching models triggers a **new pre-registered validation run** (full 500-call pass + stability + perturbation, same fixed thresholds) before any transfer work. The §5 bands and their consequences apply to the **final model in the escalation sequence**; the partially-validated and failed paths (§8) fire only after Sonnet 5's run. The 100 hand labels are reused across the escalation sequence; sequential testing against them is declared by the pre-committed escalation order (cheapest first), a fixed decision tree, not model selection on the labels. Model-ID note: 4.6-and-later model IDs are dateless but pinned snapshots per Anthropic's model versioning documentation; both judge IDs satisfy the version-pin rule.

**Self-preference caveat:** a Claude judge scoring thinking text from Claude-family agents among the 13 is a potential confound in the primary agent stratification (§7), not merely a generic bias risk; stated as a limitation in the writeup, unmeasured in v2. Cross-model agreement checks test a claim v2 does not make; future work.

**Cost:** est. tokens/unit and total run cost live in `cost-budget.md`. Judge prompts are frozen artifacts (`judge-prompts/`) — v1 of each prompt committed before the first validation run; later prompt versions are new pre-registered runs, not silent replacements. Pre-freeze review provenance for the paraphrases is logged in `judge-prompts/README.md`. **Validation-vs-deployment length shift, named:** validation runs on ~90-token own-corpus blocks; leaderboard blocks may be substantially longer. κ validated at one input-length distribution and applied at another is a stated limitation.
...
- **Sample:** decided by pre-committed cost rule, not by branch. **Cost ceiling C = $250, frozen here** (committed before adapter measurements exist — the branch condition is fixed before the data that could game it). Rule: (1) build the adapter; measure realized units-per-trajectory U and median block tokens B on the 13-agent population; (2) project full-population judge cost at the **validated judge model's** batch rates (Haiku 4.5 by default; Sonnet 5 if the §5 escalation fired); (3) projected ≤ $250 → **full population**; (4) else → **subsample** at the largest task count in {200, 100, 50} whose projection fits C — random by task, all 13 population agents per task retained (the matrix structure is the method). Reference: at mid-mid assumptions (U=40, B=300) with the frozen prompt scaffold (S≈3,000 tokens), the 13-agent full bucket projects ~$494 at Haiku batch rates and exceeds C; the expected outcome under the rule is subsample at 200 tasks (~$198). Full bucket fits only if realized U ≤ 20 at B=300. Prompt caching is not counted: the scaffold is below Haiku 4.5's 4,096-token minimum cacheable length. Details in `cost-budget.md`.
- **Q1 sample floor.** The subsample rule is bounded below by a floor on task count. The floor is defined on the judge scale: the smallest average gap between failed and resolved trajectories on the same task that exceeds labeler noise, where labeler noise is the typical size of my own disagreement with myself on a blind relabel of a subset of the validation sample (Phase 1, 20-label checkpoint, before any judge run; subset size 20, drawn by the sampling script's seed, relabeled after a gap of at least 48 hours; mean absolute disagreement recorded). Rationale: a one-level gap is enough for a reviewer sorting trajectories; adjusting a model or a harness requires detecting shifts of a few tenths, and that resolution is the claim v2 is built to earn. The floor is converted to a task count at the §7 measurement step, using measured within-task spread and contested-task fraction, at 80% power, α = 0.05 (conventional values, not derived), before any judge spend. Sampling rule with the floor: largest count in {500, 200, 100, 50} that fits C and is not below the floor. If the floor exceeds C, C rises to the floor's projected cost and the rise is logged in §9. If the floor exceeds 500 tasks, the study is recorded as underpowered for the stated effect before the judge runs, Q1 runs at 500, and its result is reported as a sensitivity bound, not a test.
...
| 2026-09-07 | §4 step 5, §7 | Self-consistency relabel recorded (not a deviation — protocol step completed as registered). First pass completed 2026-09-05; blind relabel of the 20-unit seeded subset completed 2026-09-07; the ≥48 h gate was satisfied. **Mean absolute disagreement = 0.35** (this is mean absolute disagreement, not κ). Distribution: 14 exact, 5 off-by-one, 1 off-by-two; exact-agreement rate 70%. The 2-point unit is session `afd47647-0247-460d-befa-b8e84c62493e`, block 2, first pass 1 → relabel 3. Per-unit `abs_diff` is committed in `relabel-scores.csv` (root), so the figure is recomputable from committed data alone. | Registered in §4 step 5 and §7: labeler noise must be measured before any judge run. | **Binding:** `labels.csv` first-pass scores remain the reference labels; the relabel is measurement only and changes no label. **Function:** 0.35 sets the Q1 sample floor per §7 (labeler-noise term), and is reported as the human-vs-human reference alongside κ in W1. |
| 2026-09-07 | §5 | Clarification, not a change: §5 left two analysis micro-decisions open; both were fixed before any κ was computed, in the header of the committed script (`methodology/scripts/compute_kappa.py`, PR #22): (a) judge score for κ = v1-pass1 (first pass of the deployment prompt); (b) CI = 95% bootstrap over units, 10,000 resamples, percentile method, seed 27. | §5 fixes the metric and the band thresholds but not which pass feeds κ or how the CI is constructed; committing both before computation removes any post-hoc choice. | None — closes ambiguity only. |
| 2026-09-07 | §5 | Clarification, not a change: §5 restricts the judge-vs-deterministic Spearman ρ to "files with `n_blocks_attributed ≥ 3`" without saying which population of blocks is counted. Fixed as: files with ≥ 3 **validation-sample** blocks attributed to them by v1's attribution output (the three-tier rule's `Attribution.file_paths` list, i.e. the file_paths v1 assigns to a thinking block whichever tier produced them). Blocks with empty `file_paths` are excluded from per-file aggregation; a block attributing to multiple files counts toward each file. Fixed before any ρ was computed; implemented in `methodology/scripts/compute_rho.py`. | Only the 100 sampled units carry judge scores, so the per-file mean judge score is necessarily over sample blocks; the small-N floor must count the same population or the restriction would be applied to a different quantity than the one it guards. | None — closes ambiguity only. |
...
- [x] Sampling script committed (§3, includes Q1 split seed per §7) — 2026-08-23
- [x] Judge prompt v1 + 2 paraphrases committed to `judge-prompts/` (§5) — 2026-08-29
